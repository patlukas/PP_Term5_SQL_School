...
class Sprawdziany(Methods):
    ...
    def show_frame(self) -> None:
        ...
        tk.Label(master=frame, text="Zajęcia").pack()
        self._create_table(frame, self.__list_labels, self.__rows, self.__frame_edit, self.__frame_del_row).pack()
        tk.Button(master=frame, text="Dodaj nowe zajęcia", command=self.__frame_add).pack()
        frame.pack()
    ...
    def __frame_add(self):
        for x in self.__window.winfo_children():
            x.destroy()
        frame = self._create_frame_edit_or_add(self.__window, "Dodanie sprawdzianu",
    ...
    def __frame_edit(self, index: int):
        for x in self.__window.winfo_children():
            x.destroy()
        frame = self._create_frame_edit_or_add(self.__window, "Edycja sprawdzianu",
